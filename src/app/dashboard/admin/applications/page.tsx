                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <div className="flex items-center gap-2 mb-0.5">
                        <p className="font-semibold text-stone-900">{app.first_name} {app.last_name}</p>
                        {statusBadge(app.status)}
                      </div>
                      <p className="text-sm text-stone-500">{app.email} · {app.years_experience} yrs experience</p>
                      <p className="text-xs text-stone-400 mt-0.5">Applied {formatDate(app.created_at)}</p>
                    </div>
                    <div className="flex flex-wrap gap-1 max-w-[200px] justify-end">
                      {app.cuisine_specialties?.slice(0, 2).map((c: string) => (
