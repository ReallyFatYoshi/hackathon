      ...
      <div className="py-16 md:py-20" style={{ background: 'var(--parchment)', borderBottom: '1px solid var(--border)' }}>
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-start gap-5">
            ...
            <div>
              <p className="text-xs uppercase tracking-[0.2em] font-medium mb-3" style={{ color: 'var(--gold)' }}>For Chefs</p>
              <h1 className="font-display text-[clamp(2.5rem,6vw,5rem)] font-light leading-tight" style={{ color: 'var(--ink)' }}>
                Open <span className="italic">Events</span>
              </h1>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {!events || events.length === 0 ? (
          <div className="text-center py-28">
            ...
            <p className="font-display text-2xl font-light mb-2" style={{ color: 'var(--ink)' }}>No open events right now</p>
            <p className="text-sm" style={{ color: 'var(--warm-stone)' }}>Check back soon — new events are posted regularly.</p>
          </div>
        ) : (
              ...
              <div key={event.id} className="bg-white rounded-2xl border p-6 hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5" style={{ borderColor: 'var(--border)' }}>
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  ...
                  <Link href="/login" className="flex items-center gap-1.5 text-sm font-semibold transition-colors hover:text-[#A0621A] shrink-0" style={{ color: 'var(--gold)' }}>
                    Apply as Chef <ArrowRight className="h-4 w-4" />
                  </Link>
                </div>
              </div>
            ))}
        ...
        <div className="rounded-2xl p-8 border" style={{ background: 'var(--ink)', borderColor: 'rgba(255,255,255,0.08)' }}>
          <div className="flex flex-col sm:flex-row items-center gap-6 justify-between">
            <div>
              <p className="font-display text-2xl font-light text-white mb-1">Are you a professional chef?</p>
              <p className="text-sm" style={{ color: 'var(--muted)' }}>Apply to join our verified network and start getting booked.</p>
            </div>
