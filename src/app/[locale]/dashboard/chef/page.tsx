...
export default async function ChefOverviewPage() {
  ...
  const t = await getTranslations('chefDashboard')
    ...
    <div className="space-y-8">
        ...
        <Card data-tour="chef-status" className="border-amber-200 bg-amber-50">
          <CardContent className="py-5 flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-amber-700">{t('noApplicationDesc')}</p>
            </div>
            <Link href="/apply"><Button>{tCommon('applyNow')}</Button></Link>
          </CardContent>
        </Card>
      )}
        ...
        <Card className="border-amber-200 bg-amber-50">
          <CardContent className="py-5">
            <div className="flex items-center gap-3 mb-1">
              ...
              <p className="font-semibold text-amber-800">{t('underReview')}</p>
            </div>
            <p className="text-sm text-amber-700">{t('underReviewDesc')}</p>
          </CardContent>
        </Card>
      )}
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <p className="font-semibold text-blue-800">{t('interviewScheduled')}</p>
                </div>
                <p className="text-sm text-blue-700">
                  {formatDateTime(interview.scheduledAt.toISOString())}
                </p>
                <p className="text-xs text-blue-600 mt-0.5">{t('interviewInstruction')}</p>
              </div>
      ...
      {application?.status === 'rejected' && (
        <Card className="border-red-200 bg-red-50">
          <CardContent className="py-5">
            <p className="font-semibold text-red-800">{t('notApproved')}</p>
            <p className="text-sm text-red-700 mt-0.5">
              {t('notApprovedDesc')}
              {application.adminNotes && ` ${t('note')} ${application.adminNotes}`}
            </p>
        ...
        <>
          ...
          <div data-tour="stats-grid" className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <div className="bg-white rounded-2xl border p-6" style={{ borderColor: 'var(--border)' }}>
              ...
              <p className="text-sm" style={{ color: 'var(--warm-stone)' }}>{t('eventsCompleted')}</p>
            </div>
            <div className="bg-white rounded-2xl border p-6" style={{ borderColor: 'var(--border)' }}>
              ...
              <p className="text-sm" style={{ color: 'var(--warm-stone)' }}>{t('averageRating')}</p>
            </div>
            <div className="bg-white rounded-2xl border p-6" style={{ borderColor: 'var(--border)' }}>
              ...
              <p className="text-sm" style={{ color: 'var(--warm-stone)' }}>{t('activeBookings')}</p>
            </div>
          </div>
            ...
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Link href="/dashboard/chef/bookings" className="text-xs hover:underline" style={{ color: 'var(--gold)' }}>{tCommon('viewAll')}</Link>
                </div>
              </CardHeader>
              <CardContent>
                {bookings.length === 0 ? (
                  <div className="text-center py-8" style={{ color: 'var(--muted)' }}>
                    ...
                    <Link href="/dashboard/chef/events">
                      <Button size="sm" className="mt-3">{tCommon('browseEvents')}</Button>
                    </Link>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {bookings.map((b) => (
                      <div key={b.id} className="flex items-center justify-between p-3 rounded-lg" style={{ background: 'var(--parchment)' }}>
                        ...
                        {statusBadge(b.bookingStatus, tBadges)}
                      </div>
                    ))}
                  </div>
                )}
              ...
              <CardContent className="space-y-3">
                <Link href="/dashboard/chef/events" className="block">
                  <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-stone-50 border transition-colors" style={{ borderColor: 'var(--border)' }}>
                    ...
                    <div>
                      <p className="text-sm font-medium" style={{ color: 'var(--ink)' }}>{t('browseOpenEvents')}</p>
                      <p className="text-xs" style={{ color: 'var(--warm-stone)' }}>{t('findEventsDesc')}</p>
                    </div>
                  </div>
                </Link>
                <Link href="/dashboard/chef/profile" className="block">
                  <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-stone-50 border transition-colors" style={{ borderColor: 'var(--border)' }}>
                    ...
                    <div>
                      <p className="text-sm font-medium" style={{ color: 'var(--ink)' }}>{t('updateProfile')}</p>
                      <p className="text-xs" style={{ color: 'var(--warm-stone)' }}>{t('keepProfileFresh')}</p>
                    </div>
                  </div>
                </Link>
              </CardContent>
